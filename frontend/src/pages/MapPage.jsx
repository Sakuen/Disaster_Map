...
export default function MapPage() {
  ...
  // Fetch base data
  useEffect(() => {
    // Only fetch once, we'll filter on the client for smooth UI,
    // Alternatively, we could fetch when filters change, but DeckGL handles large data well.
    // However, the backend is now updated to support max_mag, min_depth etc.
    // To keep it smooth, we fetch based on the slider filters.
    const url = `${API_URL}?start_year=${MIN_YEAR}&end_year=${MAX_YEAR}&min_mag=${filters.min_mag}&max_mag=${filters.max_mag}&min_depth=${filters.min_depth}&max_depth=${filters.max_depth}`;
  ...
  }, [filters.min_mag, filters.max_mag, filters.min_depth, filters.max_depth]);
  ...
  const layers = [
    new ScatterplotLayer({
      ...
      getFillColor: d => {
        ...
        if (clickedInfo && clickedInfo.properties.id === d.properties.id) return [255, 255, 255, 255]; 

        if (type === 'tsunami') {
          return [0, 200, 255, alpha]; 
        } else if (type === 'volcano') {
          return [200, 0, 255, alpha]; 
        } else {
          // Earthquakes
      ...
      {clickedInfo && (
        <div className="overlay-panel details-panel">
          ...
          <div className="details-meta" style={{display: 'flex', gap: '15px', marginBottom: '15px', color: '#c5c6c7', fontSize: '14px'}}>
            ...
            <span><strong>Country:</strong> {clickedInfo.properties.country}</span>
          </div>
